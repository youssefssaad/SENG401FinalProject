...
function Expenses() {
  ...
  const [totalBudget, setTotalBudget] = useState(JSON.parse(localStorage.getItem('totalBudget')) || 0);
  const [tempExpenses, setTempExpenses] = useState(JSON.parse(localStorage.getItem('tempExpenses')) || {});
  const [newExpenseAmount, setNewExpenseAmount] = useState("");
  const [newExpenseCategory, setNewExpenseCategory] = useState("");
  const [expenseIDs, setExpenseIDs] = useState([]);
  const [categoryToId, setCategoryToId] = useState(JSON.parse(localStorage.getItem('categoryToId')) || {});
  ...
  const ensureCategoryExists = async (newCategoryName, oldCategoryName = null) => {
    const jwtToken = localStorage.getItem('jwtToken');

    // If oldCategoryName is given, then just update the existing category
    if (oldCategoryName) {
      const response = await fetch(`http://localhost:8080/api/categories/updateByName/${oldCategoryName}`, {
        method: "PUT",
        headers: {
  ...
  const handleRemoveExpense = async (category) => {
    try {
      ...
      const response = await fetch(`http://localhost:8080/api/expenses/remove/${expenseId}`, {
        method: "DELETE",
        headers: {
          'Authorization': `Bearer ${jwtToken}`
        },
      ...
      const categoryUsed = Object.keys(expenses).some(key => key !== category && expenses[key].category === expenses[category].category);

      if (!categoryUsed) {
        // If the category is not used by any other expense, delete the category
        const categoryResponse = await fetch(`http://localhost:8080/api/categories/removeByName/${category}`, {
          method: "DELETE",
          headers: {
            'Authorization': `Bearer ${jwtToken}`
          },
      ...
      setGoalFields(prevGoalFields => {
        const newGoalFields = prevGoalFields.filter(goalField => goalField.goal !== category);
  ...
  const handleIncrease = async (category) => {
    ...
    const goalField = goalFields.find(goalField => goalField.goal === category);
    if (goalField) {
      const goalAmount = (totalBudget * goalField.percentage) / 100;
      if (newAmount > goalAmount) {
        alert("You are about to exceed your goal for this category");
      }
    }
  ...
  const handleSimulatePurchase = async () => {
    ...
    if (newAmount < 0) {
      alert("You have reached 0 for this category, you cannot make any more purchases to decrease the amount further");
      return;
    }

    if (newAmount < 10) { // Change this value to adjust the threshold
      alert("You are getting close to 0 for this category");
    }

    setTempExpenses((prevState) => ({ ...prevState, [newExpenseCategory]: newAmount }));
    setExpenses((prevState) => ({ ...prevState, [newExpenseCategory]: newAmount }));
    ...
    if (storedUser && storedUser.token) {
      ...
      fetch(`http://localhost:8080/api/expenses/user/${userID}`, {
        ...
        .then((expensesData) => {
          ...
          console.log("What is the fetchedCategoryID " + fetchedCategoryToId.id);
        ...
        <ul>
          {Object.entries(expenses).map(([category, amount]) => (
              <li className="expenses-item-list" key={category}>
                {category}
                <button className="increase-button" onClick={() => handleIncrease(category)}>+</button>
                <span
